...
//data_cd -> complex<double> data_dd double
data_c  in_data[NUM_ROW][NUM_COLUM];
data_c  out_data[NUM_COLUM][NUM_ROW*4];
data_d  in_coeff[NUM_COEFF];

data_cd mat_out[NUM_COLUM][NUM_ROW*4];
int Num = NUM_ROW;

//used to store data from matlab file as the input real and imag of Top
data_d data_in_real[NUM_ROW][NUM_COLUM];
data_d data_in_imag[NUM_ROW][NUM_COLUM];
//used to store the result  real and imag from Matlab
data_dd data_out_real[NUM_COLUM][NUM_ROW*4];
data_dd data_out_imag[NUM_COLUM][NUM_ROW*4];
//uesd to store the coeff from Matlab as the input of Top
...
{
	//To read the coeff
	std::vector<data_dd>  coeff_m = readFile("coeff.bin");

	for (int i = 0; i < NUM_COEFF; i++)
	{
			in_coeff[i] = (float)(coeff_m[i]);
	}
...
//	cout << endl;
    ...
    std::vector<data_dd>  inputTR = readFile("input_real.bin");
    std::vector<data_dd>  inputTI = readFile("input_imag.bin");
    ...
    for (int j = 0; j < NUM_COLUM; j++)
	{
		...
		{
            data_in_real[i][j] = (float)(inputTR[i + j * NUM_ROW]);
            data_in_imag[i][j] = (float)(inputTI[i + j * NUM_ROW]);
		}
    }
...
//     	cout << "\n";
    ...
    std::vector<data_dd>  outputTR = readFile("output_real.bin");
    std::vector<data_dd>  outputTI = readFile("output_imag.bin");
